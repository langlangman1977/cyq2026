...
```bash
# Linux/macOS
node .codex/skills/planning-with-files/scripts/session-catchup.mjs "$(pwd)"
```

```powershell
# Windows PowerShell
node "$env:USERPROFILE\.codex\skills\planning-with-files\scripts\session-catchup.mjs" (Get-Location)
```
